...
def download_city_notes(id):
    for i in range(1,999):
        ...
        #如果导航页 错误，该页的游记数目为0，则以为着1-0-xxx.html 已经遍历完，结束这个城市
        if len(city_notes)==0:
            return
        for city_note in city_notes:
            try:
                city_url = 'http://mafengwo.cn%s'%city_note[6:]
    ...
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    try:
        #download home page of mainpoint
        req=urllib2.Request('http://www.mafengwo.cn/mdd/',headers=request_headers)
        response =urllib2.urlopen(req)
        htmlcontent=response.read()
        #use re model find  home page of all city
        city_home_page=re.findall('/travel-scenic-spot/mafengwo/\d{5}.html',htmlcontent)
        # via looping ,downloaded all daypaper
        for city in city_home_page:
